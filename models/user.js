...
userSchema.methods.removeFromCart = function(productId){
  const updateedCartItems = this.cart.items.filter(item => {
          return item.productId !== productId.toString();
  this.cart.items = updated.cart.items;
  return this.save();
})
}
...
//     });
//     const db = getDb();
//     return db.collection('users').updateOne({_id: new mongodb.ObjectId(this._id)}, {$set:{cart: {items:updateedCartItems}}});
